  ...
  >
    ...
    <template #footer>
      ...
      >
        ...
        <button type="button" class="btn btn-outline-secondary text-white modal-footer-btn" @click="emit('close')">
          Close
        </button>
      </div>
    </template>
...
const emit = defineEmits(['load', 'delete', 'backup-all', 'delete-all', 'open-location', 'edit-game', 'delete-game', 'close']);

const labelId = `gameSavesModalLabel_${Math.random().toString(36).slice(2, 8)}`;
